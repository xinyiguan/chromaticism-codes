...
def plot_gpr_scatter(df: pd.DataFrame, df_type: Literal["MajorMode", "MinorMode", "CombinedMode"],
                     feature: Literal["RC", "CTC", "NCTC", "r_fifths_range", "ct_fifths_range", "nct_fifths_range"],
    ...
    fig.add_trace(go.Scatter(x=Xplot, y=f_mean, mode="lines",
                             line=dict(
                                 color=color,
                                 width=3),
                             name=f"{df_type}_{feature}_GPR_fmean"))
    fig.show()
...
def experiment_interactive_gpr_CIs(combined: pd.DataFrame, major: pd.DataFrame, minor: pd.DataFrame, optimized: bool=True):
    ...
    combined_rc = gpr_model_outputs(df=combined, df_type="Combined",  feature="RC", lengthscale=10)
    combined_ctc = gpr_model_outputs(df=combined, df_type="Combined",  feature="CTC", lengthscale=10)
    combined_nctc = gpr_model_outputs(df=combined, df_type="Combined",  feature="NCTC", lengthscale=10)
    ...
    plot_gpr_scatter(df=combined, df_type="CombinedMode",feature="RC", m_outputs=combined_rc, color=scatter_colors_palette6[0], save_name="combined_nctc_handtuned")
    plot_gpr_scatter(df=combined, df_type="CombinedMode",feature="CTC", m_outputs=combined_ctc, color=scatter_colors_palette6[1], save_name="combined_nctc_handtuned")
    plot_gpr_scatter(df=combined, df_type="CombinedMode",feature="NCTC", m_outputs=combined_nctc, color=scatter_colors_palette6[2], save_name="combined_nctc_handtuned")
...
if __name__ == "__main__":
    ...
    # experiment_interactive_gpr_5thsRange(df=major)
    # experiment_interactive_gpr_5thsRange(df=minor)

    experiment_interactive_gpr_CIs(combined=piece_df, major=major, minor=minor)
